...
- [ ] `html_parser.py`: mismatched `<ol>`/`<ul>` close can leave a stale block-interruption frame that a later, unrelated list closes by coincidence — pre-existing, confirmed live during #382's own review round, distinct from #382's own fix. Docs domain, sequence behind lane-b's current queue ([#450](https://github.com/khuisman/mcp-gee-sweet/issues/450))
...
- [ ] `transfer_ownership` missing from `server://auth-status`'s `_SA_LIMITED_TOOLS` despite its own docstring documenting the OAuth-only restriction — found during #140's review. Routed to Amy ([#447](https://github.com/khuisman/mcp-gee-sweet/issues/447))
