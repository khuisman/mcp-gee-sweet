...
- [ ] Neither the AST schema nor `html_parser.py` has any concept of a blockquote — `<blockquote>`/`> quoted text` silently converts to a plain, visually indistinguishable `Paragraph`, no indent or styling of any kind. Docs domain, same conversion-pipeline surface as #334 (lane-b's current ticket) — sequence behind it rather than run in parallel ([#476](https://github.com/khuisman/mcp-gee-sweet/issues/476))
