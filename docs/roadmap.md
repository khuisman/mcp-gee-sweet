...
- [ ] `style_doc_range` rejects `link_url=null` with "Links must include at least one type" instead of clearing an existing hyperlink — found while trying to strip a dead `#slug` anchor link left over from markdown conversion (see #409). `ready-for-development`/`lane-b` — #336 shipped, so this is now the front of the docs-emitter defect chain, queued behind #402 ([#408](https://github.com/khuisman/mcp-gee-sweet/issues/408))
- [ ] Markdown-to-Doc conversion keeps GitHub/GitLab-style `#slug` heading-anchor links as dead hyperlinks instead of resolving them to Docs-native internal links (`createNamedRange`/bookmark targets) — links render correctly on the source Markdown host but go nowhere once converted. Depends on #408 (need working link-clearing/rewriting first). `ready-for-development`/`lane-b`, queued behind #408 ([#409](https://github.com/khuisman/mcp-gee-sweet/issues/409))
...
- [ ] `transfer_ownership` — transfer a file to another user — sat on-deck since 2026-07-19 with no contributor pickup; promoted to `ready-for-development`/`lane-a` once Ash's lane freed up (#422 shipped), matching Ash's recent `transfer.py` continuity ([#140](https://github.com/khuisman/mcp-gee-sweet/issues/140))
