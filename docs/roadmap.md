...
- [ ] Plain-text content (no wrapping tag) silently produces an empty doc body — queued for lane-b ([#343](https://github.com/khuisman/mcp-gee-sweet/issues/343))
- [ ] `_get_sheet_id` catches all exceptions from the sheet-lookup fetch and returns `None`, the same value it returns when the sheet genuinely doesn't exist — every caller (24 call sites) then misreports a transient API error as "sheet not found"; flagged during #380/#89's QA _(on deck — `ready-for-development`, open for outside contribution)_ ([#384](https://github.com/khuisman/mcp-gee-sweet/issues/384))
- [ ] `_AstParser.handle_endtag` pops `_list_ordered` only on a matching close tag — a mismatched `<ol>`/`<ul>` pair leaves it permanently desynced, corrupting nesting depth for every subsequent list in the document; pre-existing, confirmed live during #369/#335's review _(on deck — `ready-for-development`, open for outside contribution)_ ([#382](https://github.com/khuisman/mcp-gee-sweet/issues/382))
