...
- [ ] `sync_folder(convert_markdown=True)`: a `.md` name over ~95 bytes overflows Drive's 124-byte property cap — upload fails with `propertyLengthLimitExceeded` but Drive still creates an unmarked orphan Doc, repeated on every sync. Needs a design call on converted-Doc recognition (PR #800 follow-up) ([#805](https://github.com/khuisman/mcp-gee-sweet/issues/805))
