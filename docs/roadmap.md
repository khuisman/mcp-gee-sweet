...
- [ ] Inline-image pre-validation (#400) only checks Google's 25-megapixel ceiling, not its 50MB file-size ceiling — a large, low-compressibility image under the megapixel limit passes `check_image_bytes` silently and only fails later at the real Docs API call; `auto_downscale=True` also has no effect for this case, since the downscale path only triggers off a `check_image_bytes` error. Confirmed live during #400/PR #554's QA round 2. `ready-for-development`/`lane-b` ([#562](https://github.com/khuisman/mcp-gee-sweet/issues/562))
