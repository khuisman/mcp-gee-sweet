...
- [ ] Migrate `mcp.server.fastmcp` → `mcp.server.mcpserver` (mcp SDK v2) — was held as a watch-item while v2 was alpha; `mcp` hit 2.0.0 stable 2026-08-20, live-confirmed `fastmcp` is fully removed (not deprecated) and `mcpserver.MCPServer` exists as expected. `ready-for-development`/`lane-a` ([#175](https://github.com/khuisman/mcp-gee-sweet/issues/175))
