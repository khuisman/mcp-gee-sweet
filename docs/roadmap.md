...
- [ ] Docs version switcher never renders and each release's `mike deploy` overwrites the previous version instead of versioning alongside it — the `latest`/`develop` split from #267 isn't actually working — config/workflow half landed in PR #668; issue stays open for two post-merge ops steps (purge the stale pre-`mike` `gh-pages` tree, bridge `versions.json` until the first release) + live verification ([#667](https://github.com/khuisman/mcp-gee-sweet/issues/667))
