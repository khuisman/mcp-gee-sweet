...
- [ ] Nested bullet/numbered list depth is computed correctly but never emitted as Docs indentation — every list renders flat regardless of source nesting; `ast_to_requests` never reads `BulletItem.depth`; found alongside #335 — held back from lane-b pickup until #360 merges, since both rewrite the same `ast_to_requests` bullet-handling logic in `emitter.py` ([#336](https://github.com/khuisman/mcp-gee-sweet/issues/336))
- [ ] `emitter.py` computes Docs API insertion offsets via Python `len()` instead of UTF-16 code units — wrong for any astral-plane character (most emoji, some CJK/math symbols); fix in progress via external contributor PR #360 — blocks #372's `content.py` split until merged, since that PR touches the exact shared offset helpers being relocated ([#358](https://github.com/khuisman/mcp-gee-sweet/issues/358))
...
- [ ] When the markdown-to-Doc converter can't represent a construct (unsupported image path, thematic break `---`, or anything else unhandled), it deletes that construct's entire paragraph rather than leaving an empty block — breaks fidelity with how every CommonMark viewer degrades (paragraph boundaries hold even when an inline element fails to resolve); confirmed live across 21 real policy documents converted via `create_doc_from_file`, every one needed manual `insert_doc_text` patches to restore lost paragraph breaks. `ready-for-development`/`lane-b`, front of queue — general fix, not image-specific; doesn't require #333's image rendering to land first ([#401](https://github.com/khuisman/mcp-gee-sweet/issues/401))
...
- [ ] `style_doc_table_cells` applies `border_color`/`border_width`/`border_dash_style` uniformly to all four cell edges — no per-edge control (top/bottom/left/right independently), even though the Docs API's own `TableCellBorder` supports it; blocks the classic "signature line" (bottom-border-only) cell pattern ([#403](https://github.com/khuisman/mcp-gee-sweet/issues/403))
- [ ] No way to set custom paragraph tab stops — Docs API supports `tabStops` on `paragraphStyle` directly, wrapper doesn't expose it; needed for form-style label/value column alignment without building a full table ([#404](https://github.com/khuisman/mcp-gee-sweet/issues/404))
