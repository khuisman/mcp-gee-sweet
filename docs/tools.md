...
122 tools. All tool names are lowercase with underscores — use these exact strings with `ENABLED_TOOLS` or `--include-tools`.
...
| `sync_folder` | Sync files between a Google Drive folder and a local directory. | `folder_id`, `local_path`, `direction?`, `export_format?`, `convert_markdown?`, `skip_system_files?`, `dry_run?`, `recursive?` |
