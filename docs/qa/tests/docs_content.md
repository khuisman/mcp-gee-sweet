...
**Note:** the companion table-position-desync half of this same finding (a literal leading tab silently under-counting `cumulative_tabs`, shifting a later table's `insertTable` position) is unit-test-covered directly (`tests/test_docs_core.py::TestBulletDepthEdgeCases::test_literal_leading_tab_does_not_desync_following_table_position`) rather than re-verified live here — it's the same code path as the nesting-level fix above, and the live check already exercises the fix at its actual defect surface.

---

### TC-DOC194: An orphan `<li>` outside any `<ul>`/`<ol>` does not take the isolated-bullet-run wrap path (#434 finding 3) (unit test)

**Background:** `html_parser.py` treats `<li>` as an ordinary `_BLOCK_TAGS` entry, reachable with no enclosing `<ul>`/`<ol>` at all — `_make_bullet_item` then computes `depth = len(self._list_ordered) - 1 = -1` for such an orphan. Before this fix, `emitter.py`'s bullet-grouping loop read this raw `-1` for both the tab-string length (`"\t" * -1`, which Python happens to already collapse to `""`) and the `min_depth` routing check (`if min_depth == 0: ... else: isolated wrap path`) — since `-1 != 0`, an orphan `<li>` (alone or grouped with a same-preset sibling) took the 3-request isolated-run wrap path (insert anchor / bullets / delete anchor) purely because of the unclamped sign, even though the actual character-level result (0 literal leading tabs either way) is identical to what the plain 1-request `min_depth == 0` path already produces. Not a rendering defect — both paths converge on the same `nestingLevel: 0` — so there is no live-observable signal to check; fixed as a pure efficiency/consistency cleanup by clamping `node.depth` to a minimum of `0` everywhere it feeds `tabs`/`min_depth` in `emitter.py`'s main loop and grouping pass.

**Checks (unit test)**
- `tests/test_docs_core.py::TestBulletDepthEdgeCases::test_negative_depth_orphan_li_does_not_take_isolated_wrap_path` — a lone orphan `BulletItem(depth=-1)` produces exactly 1 `createParagraphBullets` request and no `deleteContentRange` request (the isolated-wrap path's signature)
- `tests/test_docs_core.py::TestBulletDepthEdgeCases::test_negative_depth_orphan_li_groups_with_depth_zero_sibling` — an orphan (`depth=-1`) immediately followed by a normal `depth=0` sibling of the same preset still groups into one `createParagraphBullets` call, not two, and doesn't take the wrap path either — an unclamped `min(-1, 0) == -1` would incorrectly route this pair into the wrap path despite the depth-0 member already anchoring nesting level 0 the normal way

**Result: N/A — unit-test-verified only, per TC-DOC189/TC-D249/TC-D250 convention.** No source HTML makes this fix produce different `get_doc_structure`/rendered output than the un-fixed code — both routes converge on the identical `nestingLevel: 0` result; the only difference is 2 fewer wasted API requests per orphan `<li>` run. `uv run python -m pytest tests/test_docs_core.py -k TestBulletDepthEdgeCases` → 4 passed locally.
