...
**Resolved by #506** (`auth.py::_is_service_account_credential`): `spreadsheet_lifespan` now classifies the resolved credential's actual type (`service_account.Credentials` / `compute_engine.Credentials` → service account; `google.oauth2.credentials.Credentials` → real user, whether reached via OAuth or a user-backed ADC session) and sets `SpreadsheetContext.is_service_account_identity` accordingly — independent of which `auth_method` reached it. `auth_method` itself is left unchanged (still `"adc"` when that's how the credential was obtained — it isn't rewritten to `"service_account"`); `_auth_status_json` reads the new flag alongside `auth_method` to decide whether to report the SA limitations, and swaps the quota category's `alternatives` text to something an ADC caller can actually act on (point ADC at a user credential) instead of the circular "switch to ADC."
