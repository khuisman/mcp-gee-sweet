...
**Follow-up (#698):** the "bumped on every `mark_dirty`" claim above was not actually true — `SheetDataCache.mark_dirty(spreadsheet_id)` with no `sheet_id` (the form every `sheets/data.py` write tool calls) went through a raw `UPDATE ... LIKE` with no epoch bump, so the guard never fired for the invalidations it was meant to catch. #698 routed both prefix-invalidation paths through a shared `_BaseCache._mark_dirty_prefix` that records the invalidation, and extended the guard to `DriveFolderCache` (`list_files` now snapshots the epoch before its Drive call). Extending it to the Drive namespace made the "deliberately coarse" tradeoff above much costlier — any `mark_dirty` on any folder (every upload/move/rename) would throw away every in-flight `list_files` store for every other folder — so the guard is no longer per-instance: `_BaseCache` records the epoch at which each exact key, each `<id>:` prefix, and the whole namespace was last invalidated, and a store is skipped only if an invalidation *covering its own key* happened after its snapshot. TTL expiry observed by a `get` is no longer an invalidation at all (an entry aging out isn't a change a concurrent fetch's result predates). The size-bounded read/write pattern both caches share moved into `_BaseCache._get_sized`/`_store_sized`, which also add a keep-larger compare-and-set on `rows_fetched` and an exhaustive-fetch marker (`ROWS_EXHAUSTIVE`) — used by `DriveFolderCache` only (no `nextPageToken` and no `incompleteSearch`); a short Sheets `values().get` doesn't prove the sheet has no data past a gap of empty rows. `DocContentCache`/`CalendarCache` remain unguarded.
