...
**ADC can resolve to either identity type**, and the two have different limitations: a real user credential (`gcloud auth application-default login`) has full personal Drive access, while a service-account-backed one (a GCE/Cloud Run/GKE attached metadata identity, or `GOOGLE_APPLICATION_CREDENTIALS` pointed at a service account key) has the same restrictions as Method A above. `server://auth-status` reports which one is actually active — `auth_method` stays `"adc"` either way, but `is_service_account_identity` distinguishes them.
