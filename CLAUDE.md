...
**Sheets API `TextFormatRun.startIndex` is also UTF-16 code units** (confirmed via the API reference, same gotcha class as the Docs API note above): `update_cells` (`tools/sheets/data.py`) accepts a rich-text cell as a list of `{"text": str, "hyperlink": str | None}` run dicts for partial (rich-text) hyperlinks within a single cell (issue #89) — `_build_rich_text_cell` computes each run's `startIndex` via `_utf16_len` (`sheets/helpers.py`), not `len()`, so a run following an astral-plane character (emoji, some CJK/math symbols) lands at the correct offset. Plain (non-list) cells in the same `data` array still go through the original `values().update()` USER_ENTERED path unchanged; only cells that are rich-text lists trigger a second `batchUpdate` pass with `updateCells` requests scoped to just those cells.
