...
Aziz, Amy, and Joy aren't lane-paired — they operate at release cadence or ad hoc, not per-ticket, so they have no `Partner` and no dedicated MCP server (see §2). Kai isn't lane-paired either, and its "worktree" is the main checkout itself rather than one of the seven `.claude/worktrees/*` slots — see §1.

If the argument doesn't match one of these eight (case-insensitively), stop and say so.
