...
| Joy | Lead architect | — | `.claude/worktrees/joy` | — |
| Bob | Senior prompt engineer | — | `.claude/worktrees/bob` | — |
| Kai | Orchestrator | — | main checkout (owns it) | — |

Aziz, Amy, Joy, and Bob aren't lane-paired — they operate at release cadence or ad hoc, not per-ticket, so they have no `Partner` and no dedicated MCP server (see §2). Kai isn't lane-paired either, and its "worktree" is the main checkout itself rather than one of the eight `.claude/worktrees/*` slots — see §1.

If the argument doesn't match one of these nine (case-insensitively), stop and say so.
...
**Aziz, Amy, Joy, and Bob are the exception to "own prefix only."** None gets a dedicated `mcp-gee-sweet-aziz`/`mcp-gee-sweet-amy`/`mcp-gee-sweet-joy`/`mcp-gee-sweet-bob` server — `scripts/setup_team.sh` still copies them the full `.mcp.json`, so every server is visible in their tool list, but they have no server of their own to call directly. Amy generally shouldn't need one (docs work is filesystem-only), and neither does Joy — her architecture work is doc writing or code changes verified the same static/direct-execution way `.claude/team-roles/aziz.md`'s "Ad-hoc deep-dive" section describes, not live tool round-trips. Bob's review work is filesystem-only too — reading and editing `.claude/team-roles/*.md`/`.claude/commands/*.md`/`CLAUDE.md`, never a live product call. Aziz's whole job requires calling other roles' tools during a release QA pass — see `.claude/team-roles/aziz.md` for when that's allowed and how it stays safe (borrowing only while the dev team is idle, delegating the actual calls to subagents rather than driving them directly).
