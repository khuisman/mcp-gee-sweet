...
**Aziz, Amy, Joy, and Bob have no dedicated `mcp-gee-sweet-<name>` server of their own** — `scripts/setup_team.sh` still copies them the full `.mcp.json` for tool visibility, but none is theirs to call directly. How that plays out is role-specific (**during a release QA pass Aziz may use every `mcp-gee-sweet-*` slot** — all four lanes, Kai's, and the standalone `-oauth`/`-sa`; see `aziz.md`. Amy, Joy, and Bob stay filesystem-only) — see each one's own file rather than re-deriving it here.

**Kai's boundary is different again** — see `.claude/team-roles/kai.md`, not restated here.
