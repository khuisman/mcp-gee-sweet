...
**Aziz, Amy, Joy, and Bob have no dedicated `mcp-gee-sweet-<name>` server of their own** — `scripts/setup_team.sh` still copies them the full `.mcp.json` for tool visibility, but none is theirs to call directly. How that plays out is role-specific (Aziz borrows others' live at release-QA time, the rest stay filesystem-only) — see each one's own file rather than re-deriving it here.
