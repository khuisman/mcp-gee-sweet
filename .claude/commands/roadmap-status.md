Produce a role-based "what's left, who's doing it, and where the actual bottleneck is" report for one version target — the leaderboard Kai currently reconstructs by hand each time from `gh issue list`/`gh pr list`/worktree state (issue #622). Read-only: this command never edits labels, files, or issues, only reports on them. Runs from any checkout that has the persistent dev-team worktrees present (the main checkout, or any `.claude/worktrees/*` slot) — it shells out to `.claude/worktrees/ash` and `.claude/worktrees/jay` by relative path in step 4, which resolves the same regardless of which worktree the session itself is in.
...
- **Worktree check** (lane bucket only, when no PR match yet): `git -C .claude/worktrees/ash branch --show-current` for `lane-a` issues, `git -C .claude/worktrees/jay branch --show-current` for `lane-b` issues. If the current branch matches `issue-<N>` for one of that lane's bucketed issues, it's **in-progress** even before a PR exists.
