...
- **Pin GitHub Actions to commit SHAs** — `actions/checkout@v4`, `astral-sh/setup-uv@v5`, etc. use mutable tags; a compromised upstream repo can move the tag to malicious code. Pin each action to a specific commit SHA and enable Dependabot to keep them updated. See `decision-publishing.md` supply chain context.
